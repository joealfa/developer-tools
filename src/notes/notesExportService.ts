...
export class NotesExportService implements vscode.Disposable {
	...
	async importNotes(): Promise<boolean> {
		...
		const importUri = vscode.Uri.joinPath(workspaceFolders[0].uri, '.vscode', 'notes.json');
	...
	async enableAutoExport(): Promise<void> {
		...
		vscode.window.showInformationMessage(
			'Auto-export enabled. Notes will be saved to .vscode/notes.json automatically.'
		);
	}
