...
import { Note, CreateNoteOptions, UpdateNoteOptions, NotesFilter, NotesChangeEvent } from './types';
...
export class NotesService implements vscode.Disposable {
	...
	getByLine(filePath: string, lineNumber: number): Note[] {
		return Array.from(this.notes.values()).filter(
			(note) => note.filePath === filePath && note.lineNumber === lineNumber
		);
	}
	...
	async reanchor(
		...
		lineNumber: number,
		lineContent: string,
		surroundingContext?: { lineBefore?: string; lineAfter?: string }
