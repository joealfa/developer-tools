...
export class ComplexityAnalyzer {
	...
	calculateCognitiveComplexity(body: string, language: SupportedLanguage): number {
		...
		const noNestingPenalty = (kw: string) => ['else', 'elif', 'else if'].includes(kw);
	...
	private extractControlFlowKeywords(line: string, _language: SupportedLanguage): string[] {
		...
		if (/\belse\s+if\b/.test(line)) {
			keywords.push('else if');
		} else if (/\belif\b/.test(line)) {
			keywords.push('elif');
		} else {
			if (/\bif\b/.test(line)) {
				keywords.push('if');
		...
		if (/\bswitch\b/.test(line)) {
			keywords.push('switch');
		}
		if (/\bcase\b/.test(line)) {
			keywords.push('case');
		}
		...
		if (/\bloop\b/.test(line)) {
			keywords.push('loop');
		}
		if (/\bselect\b/.test(line)) {
