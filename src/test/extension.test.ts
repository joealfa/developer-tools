...
suite('UUID/GUID Generation Tests', () => {
	...
	test('Insert UUID command exists', async () => {
		...
		assert.ok(
			commands.includes('developer-tools.insertUuid'),
			'insertUuid command should be registered'
		);
	});
	...
	test('Insert GUID command exists', async () => {
		const commands = await vscode.commands.getCommands(true);
		assert.ok(
			commands.includes('developer-tools.insertGuid'),
			'insertGuid command should be registered'
		);
	});
	...
	test('Insert UUID Compact command exists', async () => {
		const commands = await vscode.commands.getCommands(true);
		assert.ok(
			commands.includes('developer-tools.insertUuidCompact'),
			'insertUuidCompact command should be registered'
		);
	});
	...
	test('Insert GUID Compact command exists', async () => {
		const commands = await vscode.commands.getCommands(true);
		assert.ok(
			commands.includes('developer-tools.insertGuidCompact'),
			'insertGuidCompact command should be registered'
		);
	});
	...
	test('Insert UUID Compact generates valid UUID without hyphens', async () => {
		...
		const text = editor.document.getText();
		assert.ok(
			uuidCompactRegex.test(text),
			`Generated compact UUID should be 32 hex chars: ${text}`
		);
	...
	test('Insert GUID Compact generates valid GUID without hyphens', async () => {
		...
		assert.ok(
			uuidCompactRegex.test(text),
			`Generated compact GUID should be 32 hex chars: ${text}`
		);
...
suite('Password Generator Tests', () => {
	test('Generate Password command exists', async () => {
		const commands = await vscode.commands.getCommands(true);
		assert.ok(
			commands.includes('developer-tools.generatePassword'),
			'generatePassword command should be registered'
		);
	});
...
suite('Password Generation Unit Tests', () => {
	...
	test('generatePassword with special only contains special characters', () => {
		...
		assert.ok(
			/^[!@#$%^&*]+$/.test(password),
			`Password should only contain special chars: ${password}`
		);
	});
	...
	test('generatePassword respects minimum numbers requirement', () => {
		...
		assert.ok(
			numberCount >= 5,
			`Password should have at least 5 numbers, got ${numberCount}: ${password}`
		);
	});
	...
	test('generatePassword returns empty string when no character sets selected', () => {
		...
		assert.strictEqual(
			password,
			'',
			'Password should be empty when no character sets selected'
		);
	});
...
suite('Notes Feature Tests', () => {
	test('Add Note command exists', async () => {
		const commands = await vscode.commands.getCommands(true);
		assert.ok(
			commands.includes('developer-tools.addNote'),
			'addNote command should be registered'
		);
	});
	...
	test('Edit Note command exists', async () => {
		const commands = await vscode.commands.getCommands(true);
		assert.ok(
			commands.includes('developer-tools.editNote'),
			'editNote command should be registered'
		);
	});
	...
	test('Delete Note command exists', async () => {
		const commands = await vscode.commands.getCommands(true);
		assert.ok(
			commands.includes('developer-tools.deleteNote'),
			'deleteNote command should be registered'
		);
	});
	...
	test('Show Notes Panel command exists', async () => {
		const commands = await vscode.commands.getCommands(true);
		assert.ok(
			commands.includes('developer-tools.showNotesPanel'),
			'showNotesPanel command should be registered'
		);
	});
	...
	test('Export Notes command exists', async () => {
		const commands = await vscode.commands.getCommands(true);
		assert.ok(
			commands.includes('developer-tools.exportNotes'),
			'exportNotes command should be registered'
		);
	});
	...
	test('Import Notes command exists', async () => {
		const commands = await vscode.commands.getCommands(true);
		assert.ok(
			commands.includes('developer-tools.importNotes'),
			'importNotes command should be registered'
		);
	});
	...
	test('Start Session command exists', async () => {
		const commands = await vscode.commands.getCommands(true);
		assert.ok(
			commands.includes('developer-tools.startSession'),
...
suite('Notes Types Tests', () => {
	...
	test('STORAGE_FILES has correct paths', () => {
		...
		assert.ok(
			STORAGE_FILES.NOTES_FILE === '.vscode/notes/notes.json',
			'Should have notes file path'
		);
		assert.ok(
			STORAGE_FILES.BACKUP_FILE === '.vscode/notes/notes-backup.json',
			'Should have backup file path'
		);
	});
});
